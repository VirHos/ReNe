...
class ComplexFilter:
    ...
    def __init__(self, user_pr: UserProcessor, filter_names):
    ...
    def filter_old(self, pred, user):
        ...
        dises  = [pr for pr in pred if np.array(pr['data'], dtype=np.datetime64) > np.array(ld, dtype=np.datetime64) - np.timedelta64(7, 'D')]
